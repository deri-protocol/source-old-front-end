...
function PoolBox({ wallet, version, pool, lang }) {
  ...
  useEffect(() => {
    if (pool && pool.airdrop) {
      ...
      setButtonElement(
        <a target="_blank" href={`https://app.deri.finance/#/pool/${pool.version}/${pool.chainId}/${pool.type}/${pool.address}`}>
          {lang['staking']}
        </a>
      )
    }
    ...
    <div className="pool" >
      <div className="pool-header">
        ...
        <div className='pool-label'>
          {pool.version === 'v1' && <img src={v1LabelImg} alt='v1' />}
          {pool.version !== 'v1' && lang[pool.version]}</div>
      </div>
          ...
          <div className="top-info">
            ...
            <div className="pool-detail">
              ...
              <div>
                ...
                <span className='symbol-retired'>
                  {pool.version !== 'v2' && pool.symbol}
                  {pool.version === 'v2' && 'BTCUSD,ETHUSD,BNBUSD'}
                </span>
              </div>
              <div className="apy">
                <span>{lang['apy']}</span>
