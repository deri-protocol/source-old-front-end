...
import { ERC20Factory, symbolOracleOffChainFactory } from "../../shared/contract/factory.js";
import {
  toWei,
  getLiquidity,
...
const init = (klass) => {
  ...
  klass.prototype['init'] = async function (isEstimatedApi = false) {
    ...
    if (!this.addresses || !this.activeSymbolIds) {
      [this.addresses, this.parameters, this.stateValues, this.fundingPeriod] = await Promise.all([
        this.getAddresses(),
        this.getParameters(),
...
const getStateValues = (klass) => {
  klass.prototype["getStateValues"] = async function () {
    ...
    // update
    this.stateValues = { ...this.stateValues, ...res}
...
const getDynamicEquity = (klass) => {
  klass.prototype['getDynamicEquity'] = async function() {
    ...
    const res = bg(this.stateValues.liquidity)
      .plus(
        this.symbols.reduce(
          (acc, s) =>
            acc
      ...
      .toString();
    this.stateValues.dynamicEquity = res
    return res
  }
  return klass
}
