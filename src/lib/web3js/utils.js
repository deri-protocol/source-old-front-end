...
export const toNatural = (value, num = 0) => {
  return BigNumber(value).toFixed(num).toString();
};
...
export const getPoolContractAddress = (chainId, bTokenSymbol) => {
  ...
  console.log(`getPoolContractAddress(): contract address is not found: ${chainId} ${bTokenSymbol}`);
  return [];
};

// get config by poolAddress
export const getPoolContractAddress2 = (chainId, poolAddress) => {
  chainId = normalizeChainId(chainId)
  const pools = getContractAddressConfig(DeriEnv.get()).filter(
    (c) => c.chainId === chainId
  );
  ...
  console.log(`getPoolContractAddress2(): contract address is not found: ${chainId} ${poolAddress}`);
  return [];
};
...
export const getSlpContractAddress = (chainId, poolAddress) => {
  ...
  console.log(`getSlpContractAddress(): contract address is not found: ${chainId} ${poolAddress}`);
  return {};
};
...
export const getDeriContractAddress = (chainId) => {
  ...
  console.log(`getDeriContractAddress(): contract address is not found: ${chainId}`);
  return {};
};

export const getBTCUSDPrice = async () => {
  try {
    const priceResponse = await fetch(getOracleUrl(DeriEnv.get()));
...
export const getOracleInfo = async () => {
  try {
    const url = getOracleUrl(DeriEnv.get());
