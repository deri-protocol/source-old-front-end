...
function LightChart({ interval = '1', displayCandleData, mixedChart, lang, showLoad, preload, trading }) {
    ...
    const loadData = async(symbol, from, to, hiddenLoading) => {
        ...
        const url = `${process.env.REACT_APP_HTTP_URL}/get_kline?symbol=${symbol}&time_type=${intervalRange[interval]}&from=${range[0]}&to=${range[1]}` && !hiddenLoading && showLoad(true)
    ...
    return ( < div className = 'ligth-chart-container'
            id = 'ligth-chart-container'
            ref = { chartRef } >
            <div className = { scrollClassName } > </div> {
            mixedChart && < div className = 'legend' >
            <div onClick = {
                () => switchSeriesChart(candlesChartRef.current, 'right')
            } > < span className = 'legend-option-left' > </span><span className='legend-option-right'> </span > { lang['option'] } </div> <div onClick = {
            () => switchSeriesChart(lineChartRef.current, 'left')
        } > < span className = 'legend-index' > </span>{trading.config && stripSymbol(`${trading.config.symbol}`)}</div >
        </div>} </div >
)
}
